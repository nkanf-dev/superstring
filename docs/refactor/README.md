...
> 方案基线：`/Users/nkanf/clones/superstring` HEAD `d3167e4`，2026-09-25。本目录是**架构决策 + 逐文件施工与迁移验收**的完整文档集合；实施进度见 [Implementation ledger](./IMPLEMENTATION.md)；三个阶段均已完成本地实现与自动化验证；真实环境验收仍有待补项。已落地调用图与代码地图见 [实现架构与功能证据](./13-实现架构与功能证据.md)。本文图示保留设计目标。现状事实见[业务 Wiki 首页](/Users/nkanf/docs/superstring/README.md)与[代码地图](/Users/nkanf/docs/superstring/06-代码地图与项目全貌.md)。
