...
- Effect: source+journal hooks can remain in one SQLite transaction; there is no pretend async atomic callback. Read-only replacements implement query only. The injected source resolver is shared by live consumption and ContextHandle inspection; unhandled sources still use existing SQLite authorization, and run ownership plus retained expiry remain mandatory. Remote backend storage is not implemented. Existing Web frozen-read policy is moved into a module-owned compatibility adapter so its leases/capacities/snapshots remain unchanged.
