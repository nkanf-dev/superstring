...
export function canReadRun(
  ...
  switch (owner.kind) {
    ...
    case "web_turn": {
      const row = db.query(`SELECT s.user_id FROM turns t JOIN sessions s ON s.id=t.session_id
        WHERE t.id=?`).get(owner.id) as { user_id: string } | null;
...
export function sourceAccess(
  ...
  switch (source.kind) {
    case "web_turn": {
      const row = db.query(`SELECT t.source_valid,t.context_valid,s.user_id,s.agent_id FROM turns t
        JOIN sessions s ON s.id=t.session_id WHERE t.id=?`).get(source.id) as {
        source_valid: number; context_valid: number; user_id: string; agent_id: string;
      } | null;
      return row && row.user_id === principal.userId && row.source_valid === 1 &&
        row.context_valid === 1 && (!owner.agentId || owner.agentId === row.agent_id)
        ? "available" : "revoked";
    }
    case "memory": {
    ...
    case "knowledge_document": {
      const row = db.query("SELECT content_version FROM knowledge_documents WHERE id=?")
        .get(source.id) as { content_version: number } | null;
      return principal.userId === DEFAULT_USER_ID && row &&
        String(row.content_version) === source.revision ? "available" : "revoked";
    }
    case "knowledge_grant": {
      const identity: unknown = JSON.parse(source.id);
      if (!Array.isArray(identity) || identity.length !== 2 ||
          !identity.every((part) => typeof part === "string")) return "revoked";
      if (owner.agentId && owner.agentId !== identity[1]) return "revoked";
      const grant = db.query("SELECT token FROM knowledge_grants WHERE document_id=? AND agent_id=?")
        .get(identity[0], identity[1]) as { token: string } | null;
      return principal.userId === DEFAULT_USER_ID && grant?.token === source.revision
        ? "available" : "revoked";
    }
    case "qq_observation": {
      const row = db.query(`SELECT e.agent_id,t.expires_at FROM qq_events e
      ...
      if (!row || principal.userId !== DEFAULT_USER_ID ||
          (owner.agentId && row.agent_id !== owner.agentId)) return "revoked";
      return !row.expires_at || Date.parse(row.expires_at) <= Date.parse(now)
        ? "expired" : "available";
    }
    case "qq_media": {
      const row = db.query(`SELECT n.expires_at,e.agent_id FROM qq_media_notes n
        JOIN qq_events e ON e.event_key=n.event_key WHERE n.id=?`).get(source.id) as {
        expires_at: string; agent_id: string;
      } | null;
      if (!row) return "expired";
      if (principal.userId !== DEFAULT_USER_ID ||
          (owner.agentId && owner.agentId !== row.agent_id)) return "revoked";
      return Date.parse(row.expires_at) <= Date.parse(now) ? "expired" : "available";
    }
    case "qq_speech": {
      const row = db.query(`SELECT s.agent_id,t.expires_at FROM qq_speech_log s
        LEFT JOIN qq_speech_text t ON t.speech_id=s.id WHERE s.id=?`).get(source.id) as {
        agent_id: string; expires_at: string | null;
      } | null;
      if (!row || principal.userId !== DEFAULT_USER_ID ||
          (owner.agentId && owner.agentId !== row.agent_id)) return "revoked";
      return !row.expires_at || Date.parse(row.expires_at) <= Date.parse(now)
...
export function inspectContext(
  ...
  if (status === "exact") {
    const states = stored.sources.map((source) => sourceAccess(db, source, run.owner, principal, now));
    if (states.includes("revoked")) status = "revoked";
    else if (states.includes("expired") ||
      (stored.expiresAt && Date.parse(stored.expiresAt) <= Date.parse(now))) status = "expired";
  ...
  const unavailableMedia = stored.messages.flatMap((message) => message.content.flatMap((part) =>
    part.kind === "image" ? [{
      sourceId: part.sourceId, sha256: part.sha256, reason: "media_unavailable" as const,
    }] : []));
  return {
    ...metadata,
